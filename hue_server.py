...
def get_bridge_ctx(ctx: Context) -> Tuple[Bridge, Dict]:
    ...
    return hue_ctx.bridge, hue_ctx.light_info  # type: ignore
...
def rgb_to_xy(r: int, g: int, b: int) -> List[float]:
    ...
    _r = pow(_r, 2.2) if _r > 0.04045 else _r/12.92
    _g = pow(_g, 2.2) if _g > 0.04045 else _g/12.92
    _b = pow(_b, 2.2) if _b > 0.04045 else _b/12.92

    # Convert to XYZ using the Wide RGB D65 conversion matrix
    X = _r * 0.649926 + _g * 0.103455 + _b * 0.197109
    Y = _r * 0.234327 + _g * 0.743075 + _b * 0.022598
    Z = _r * 0.000000 + _g * 0.053077 + _b * 1.035763
...
def turn_on_light(light_id: int, ctx: Context) -> str:
    ...
    try:
        ...
        return f"Light {light_id}  ({
            light_info[str(light_id)]['name']}) turned on."
    except Exception as e:
...
def turn_off_light(light_id: int, ctx: Context) -> str:
    ...
    try:
        ...
        return f"Light {light_id}  ({
            light_info[str(light_id)]['name']}) turned off."
    except Exception as e:
...
@mcp.tool()
def set_color_rgb(
        light_id: int, red: int, green: int, blue: int, ctx: Context) -> str:
    """
    ...
    try:
        ...
        if 'xy' not in light_info[str(light_id)]['state']:
            return f"Error: Light {light_id}  ({
                light_info[str(light_id)]['name']}) does not support color."
        ...
        return f"Light {light_id}  ({
            light_info[str(light_id)]['name']}) color set to RGB({red}, {
            green}, {blue})."
    except Exception as e:
...
def set_group_brightness(group_id: int, brightness: int, ctx: Context) -> str:
    ...
    try:
        ...
        # Calculate brightness percentage for user feedback
        percentage = round((brightness / 254) * 100)
        return f"Group {group_id}  ({group_name}) brightness set to {
...
@mcp.tool()
def set_group_color_rgb(
        group_id: int, red: int, green: int, blue: int, ctx: Context) -> str:
    """
    ...
    try:
        ...
        return f"Group {group_id}  ({group_name}) color set to RGB({red}, {
            green}, {blue})."
    except Exception as e:
...
def set_scene(group_id: int, scene_id: str, ctx: Context) -> str:
    ...
    try:
        ...
        logger.error(
            f"Error applying scene {scene_id} to group {group_id}: {e}")
        return f"Error: {str(e)}"
...
def create_group(
    ...
    try:
        ...
        invalid_lights = [
            lid for lid in light_ids
            if not validate_light_id(lid, light_info)]
        if invalid_lights:
        ...
        if 'success' in result[0]:
            ...
            return f"Group '{name} ' created with ID {group_id}, containing {
                len(light_ids)}  lights."
        else:
...
def quick_scene(
    ...
    try:
        ...
        if brightness is not None:
            changes.append(
                f"brightness {brightness} ({round((brightness / 254) * 100)}%)")
        if rgb is not None:
            changes.append(f"color RGB({rgb[0]}, {rgb[1]}, {rgb[2]})")
        if temperature is not None:
            changes.append(f"temperature {temperature}K")

        return f"Scene '{name} ' applied to group '{group_name} ' with {
            ', '.join(changes)}."
    except Exception as e:
...
def refresh_lights(ctx: Context) -> str:
    ...
    try:
        ...
        # Update our cache
        light_info = bridge.get_light()
        ctx.request_context.lifespan_context.light_info = light_info  # type: ignore
...
def set_color_preset(
    ...
    if preset not in presets:
        return f"Error: Unknown preset. Available presets: {
            ', '.join(presets.keys())} "
    ...
    try:
        ...
        if "ct" in presets[preset] and 'ct' not in light_info[str(light_id)][
                'state']:
            return f"Error: Light {light_id} does not support color temperature."

        # Check capability for xy color
        if "xy" in presets[preset] and 'xy' not in light_info[str(light_id)][
                'state']:
        ...
        return f"Applied '{preset} ' preset to light {light_id}  ({
            light_info[str(light_id)]['name']})."
    except Exception as e:
        logger.error(
            f"Error applying preset '{preset}' to light {light_id}: {e}")
        return f"Error: {str(e)}"
...
def set_group_color_preset(
    ...
    try:
        ...
        logger.error(
            f"Error applying preset '{preset}' to group {group_id}: {e}")
        return f"Error: {str(e)}"
...
def alert_light(light_id: int, ctx: Context) -> str:
    ...
    try:
        ...
        return f"Light {light_id}  ({
            light_info[str(light_id)]['name']}) alerted with a brief flash."
    except Exception as e:
...
def set_light_effect(light_id: int, effect: str, ctx: Context) -> str:
    ...
    try:
        ...
        return f"Set {effect_name}  on light {light_id}  ({
            light_info[str(light_id)]['name']})."
    except Exception as e:
...
if __name__ == "__main__":
    ...
    mcp.run(transport='stdio')  # Use this for direct execution
